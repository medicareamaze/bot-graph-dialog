...
class GraphDialog {
  ...
  setBotDialog() {
    ...
    this.options.bot.dialog(this.internalPath, [
      async (session, args, next) => {
        session.dialogData.data = args || session.userData || {};
  ...
  async stepInteractionHandler(session, results, next) {
    ...
    switch (currentNode.type) {
      ...
      case Node.NodeType.adaptive:
        if (session.message && session.message.value && session.message.value["stopBeforeProcessingNextStep"] && session.message.value["stopBeforeProcessingNextStep"]=='yes' ) { 
          session.send(this.generateAdaptiveCardMessage(session, currentNode));
          break;
        }
        else if(session.message && session.message.value && session.message.value["stopBeforeProcessingNextStep"] && session.message.value["stopBeforeProcessingNextStep"]=='no') {
        //session.send(this.generateAdaptiveCardMessage(session, currentNode));
        return next();
        }
        else {
  ...
  generateAdaptiveCardMessage(session, node) {
    ...
    var strAttachments = JSON.stringify(node.data.attachments);
    strAttachments = strformat(strAttachments, session.dialogData.data);
    var attachmentsContent = JSON.parse(strAttachments);
    var msg = new builder.Message(session);
    if (node.data.attachmentLayout) msg.attachmentLayout = node.data.attachmentLayout;
    attachmentsContent.forEach(function (attachment) {
        msg.addAttachment({
            contentType: "application/vnd.microsoft.card.adaptive",
            content: attachment.content
        });
    });
    if (session.message && session.message.value && session.message.value["stopBeforeProcessingNextStep"] ) { 
      session.message.value["stopBeforeProcessingNextStep"]=false;
    }
    return msg;
